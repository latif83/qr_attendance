...
export const EnterPasswords = ({ setAuthenticate, setAuthDone }) => {
  ...
  useEffect(() => {
    const authenticateUser = async () => {
      try {
        ...
        const response = await fetch("/api/attendance/clockin/auth", {
          method: "POST",
    ...
    <div className={`${styles.container} flex items-center justify-center`}>
      <div className="w-full max-w-2xl mx-auto bg-gray-50 rounded shadow p-6">
        <div className="flex justify-between mb-5">
          <h1 className="font-semibold">Enter your Password</h1>
          <FontAwesomeIcon
            icon={faTimes}
